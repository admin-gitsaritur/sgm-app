...
export interface Column<T> {
    ...
    header: string
    /** Largura da coluna (ex: "200px", "2fr", "auto") */
    width?: string
    ...
    /** Variante visual da célula: 'primary' | 'secondary' | 'none'. 
     *  Por padrão, a 1ª coluna é 'primary' e as demais são 'secondary'. 
     *  Use 'none' para desabilitar o styling automático. */
...
export interface DataTableProps<T> {
    ...
    /** Esconder busca */
    hideSearch?: boolean
    /** Esconder área inteira da toolbar (search + botões) */
    hideToolbar?: boolean
    /** Remove a scrollbar interna forçando a tabela a crescer até onde precisar */
    noScroll?: boolean
    /** Botão de ação (slot) */
    actionButton?: React.ReactNode
    /** Conteúdo extra renderizado entre o toolbar (search) e a tabela */
    ...
    /** Linha de resumo/somatório exibida ao final da tabela (antes do rodapé de paginação) */
...
export function DataTable<T>({
    ...
    // Obter valor de uma célula (suporta paths como "user.name")
    ...
    return (
        <div className={cn("flex flex-col h-full", className)}>
            {/* Toolbar */}
            {!hideToolbar && (!hideSearch || actionButton) && (
            ...
            {/* Slot: conteúdo entre search e tabela (filtros, etc.) */}
            {afterSearch && <div className="mb-6">{afterSearch}</div>}

            {/* Card da Tabela */}
            <div className="flex-1 bg-white rounded-[24px] border border-stone-200 soft-shadow flex flex-col min-h-0 relative overflow-hidden">
                ...
                {tabs && tabs.length > 0 && (
                    <div className="relative border-b border-stone-100 px-6 pt-2 shrink-0 bg-white z-20">
                        ...
                        <div
                            className="absolute bottom-0 h-0.5 bg-primary tab-underline"
                            style={{
                ...
                {/* Tabela */}
                {!loading && data.length > 0 && (
                    <div className={cn("overflow-x-auto custom-scroll relative", noScroll ? "" : "flex-1 overflow-y-auto")}>
                        <table className="w-full text-left border-collapse min-w-[600px]">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {columns.map((column, colIndex) => {
                                                ...
                                                // Auto-apply cell variant: first col = primary, rest = secondary
                                                const variant = column.cellVariant ?? (colIndex === 0 ? 'primary' : 'secondary')
                                                const rawContent = column.render
                                                    ? column.render(value, row, rowIndex)
                                                    : value != null ? String(value) : "-"

                                                // Auto-wrap: primitivos (string/number) recebem Primary/Secondary
                                                // JSX (React element) = dev já tratou o estilo → sem wrapper
                                                // cellVariant 'none' = sem wrapper (ex: ProgressBar, badges)
                            ...
                            {/* Summary/Totals row */}
                            {summaryRow && (
                                <tfoot className="bg-stone-50/80 border-t-2 border-stone-200 sticky bottom-0 z-10">
                                    {summaryRow}
                                </tfoot>
                            )}
                        </table>
                ...
                {/* Rodapé - Sempre visível quando há dados */}
                {!loading && data.length > 0 && (
                    <div className="px-6 py-5 border-t border-stone-100 bg-stone-50/30 flex items-center justify-between shrink-0 rounded-b-3xl">
                        ...
                        {pagination && (
                            <div className="flex items-center gap-2">
                                <button
                                    className={cn(
                                        "h-9 w-9 flex items-center justify-center rounded-lg border border-stone-200 bg-white text-stone-400 transition-colors",
                                        pagination.page === 1
                                    ...
                                    {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={cn(
                                                    "w-9 h-9 rounded-lg text-xs font-semibold flex items-center justify-center transition-all",
                                                    isActive
...
// ============================================================================
// COMPONENTES AUXILIARES PARA CUSTOMIZAÇÃO
// ============================================================================

/** Wrapper para texto principal (bold, cor marrom, laranja no hover) */
export function DataTableCellPrimary({
    ...
    subtitle?: string
    /** Indica estado muted/desativado (opacidade reduzida) */
    muted?: boolean
}) {
    return (
...
/** Wrapper para texto secundário (cinza) */
export function DataTableCellSecondary({
    children,
    className,
    muted = false
}: {
    ...
    return <span className={cn("text-sm text-stone-600", muted && "opacity-40", className)}>{children}</span>
}
...
export function DataTableStatusBadge({
    ...
    return (
        <span className={cn(
            "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[11px] font-semibold border",
            styles[variant]
        )}>
...
/** Badge colorida simples */
export function DataTableBadge({
    children,
    color = "gray",
}: {
    children: React.ReactNode
    color?: "gray" | "orange" | "green" | "red" | "blue" | "yellow" | string
}) {
    const colorClasses: Record<string, string> = {
        ...
        yellow: "bg-saritur-yellow/10 text-amber-700",
    }

    const classes = colorClasses[color] || colorClasses.gray

    return (
        <span className={cn("px-3 py-1.5 rounded-xl text-xs font-bold inline-block", classes)}>
            {children}
        </span>
    )
}
...
export function DataTableCount({ count }: { count: number }) {
    ...
    return (
        <span className="px-2 py-1 rounded-xl text-xs font-medium bg-amber-100 text-amber-800">
...
export function DataTableAvatar({
    ...
    return (
        <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-xs font-bold text-stone-500 border border-white shadow-sm">
...
export function DataTableActions({ onClick }: { onClick?: () => void }) {
    return (
        <button
            ...
            className="p-2 rounded-lg text-stone-300 hover:text-brown hover:bg-stone-100 transition-colors"
        >
